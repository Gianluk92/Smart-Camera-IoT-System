...
void get_periodic_handler(void* request, void* response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
	...
	sprintf(message, "%u", doorwin_value);
	length = strlen(message);
	memcpy(buffer, message, length);
...
void post_periodic_handler(void* request, void* response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
	...
	len = REST.get_post_variable(request,"doorwin_set",&val);
	
	if( len > 0 ){
    	new_value = atoi(val);
    	if(new_value == 1 || new_value == 0)	
    		doorwin_value = new_value;
    	REST.set_response_status(response, REST.status.CREATED);
    } 
    else 
    	REST.set_response_status(response, REST.status.BAD_REQUEST);
...
int used = 0;
void event_post_handler(){
	if(doorwin_value == 1 || used == 1){
		REST.notify_subscribers(&doorwin);
		used = doorwin_value;
	}
}
...
PROCESS_THREAD(server, ev, data)
{
	...
	rest_activate_resource(&doorwin, "doorwin");
	while(1) {
   		PROCESS_WAIT_EVENT();
	}
	PROCESS_END();
